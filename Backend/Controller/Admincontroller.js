...
// ======================================================
// 13. ISSUE REPORTS MANAGEMENT (ADMIN)
// ======================================================
...
exports.getAllIssues = async (req, res) => {
    try {
        ...
        const issues = await IssueReport.find(filter)
            .populate("driver", "Name PhoneNumber Email driverReferenceId CountryCode")
            .sort({ createdAt: -1 });
...
exports.getPendingIssues = async (req, res) => {
    try {
        const issues = await IssueReport.find({ status: "Pending" })
            .populate("driver", "Name PhoneNumber Email driverReferenceId CountryCode")
            .sort({ createdAt: -1 });
...
exports.updateIssueStatus = async (req, res) => {
    try {
        ...
        if (!["Pending", "In Review", "Resolved", "Dismissed"].includes(status)) {
            return res.status(400).json({
                success: false,
                message: "Status must be one of: Pending, In Review, Resolved, Dismissed",
            });
        }
...
exports.deleteIssue = async (req, res) => {
    try {
        const { id } = req.params;
        const issue = await IssueReport.findByIdAndDelete(id);
        if (!issue) {
            return res.status(404).json({
                success: false,
                message: "Issue report not found",
